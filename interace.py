...
def preprocess(video_paths, frames=16, resolution=224):
    ...
    for p in video_paths:
        ...
        for i in idxs:
            ...
            img = img[..., ::-1]/255.0  # BGR→RGB, normalize 0–1
            frames_list.append(img)
        cap.release()
        clip = np.stack(frames_list, axis=0)  # T×H×W×3
        clips.append(clip)
    # shape → (1, V, C, T, H, W)
    arr = np.stack(clips, axis=0)
    arr = np.transpose(arr, (0,1,4,0,2,3))
    return torch.tensor(arr, dtype=torch.float32)  # adjust dtype
...
if __name__=="__main__":
    ...
    p.add_argument("--videos", nargs="+", required=True,
                   help="Paths to each view’s video")
